...
namespace BGSerialLib
{
    ...
    public class BGSerialPort : IDisposable
    {
        ...
        private int bgTimeOut = 10; //Set receive default timeout to 10 seconds
        ...
        /// <summary>
        /// Event fired whenever new B&G data frame is acquired from the network.
        /// </summary>
       ...
       #region Local Methods
        ...
        /// <summary>
        /// Search a byte array a for data, starting at start, for char until end
        /// </summary>
        /// <param name="array">byte array</param>
        /// <param name="data">int< char/param>
        /// <param name="start">int< start/param>
        /// <param name="end">int< end/param>
        /// <returns>Search byte[]array for byte data, int start, int end</returns>
        private int SearchBytes(byte[] array, byte data, int start, int end)
        {
            while (start < end)
            {
                if (array[start] != data)
        ...
        {
            get { return csTotalBytes; }
            set { csTotalBytes = value; }
        }
        ...
        private void buildFrame()
        {
            if (endBuffer - startBuffer > 0)  // Ok, lets make sure that we have data in sBuffer
            {
                ...
                // Minimum valid header-frame size is 5 bytes
                ...
                if ((endBuffer - startBuffer) >= 5)
                {
                    ...
                    else
                    {
                        ...
                        {
                            startBuffer++;      // Adjust buffer pointer by 1 to try a re-sync
                            reScanFlag = true;  // Force a re-scan
                        }
                        else { reScanFlag = false; } // Not sure how to handle this case ********
                    }
                }
                else
        ...
        private void parser(byte[] ppData)
        {
            ...
            else
            {
                ...
                // we may need to fix this.... header did pass a CS to get here??
                ...
                // lets send it anyways with a checksum error
